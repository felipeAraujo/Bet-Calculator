...
class Bet_Calculator(object):
    ...
    def profit_if_team1_wins(self, team1_bet_value, bet_team1_house1 = True):
        value_of_bet_on_team1 = Decimal(team1_bet_value)
        if value_of_bet_on_team1 > self.cash_to_bet:
            raise Exception("Bet Value bigger than the cash to bet defined in class")

        if bet_team1_house1:
            return self._decimal_team1_house1*value_of_bet_on_team1 - self.cash_to_bet
        else:
            return self.decimal_team1_house2*value_of_bet_on_team1 - self.cash_to_bet

    def profit_if_team2_wins(self, team2_bet_value, bet_team1_house1 = True):
        value_of_bet_on_team2 = Decimal(team2_bet_value)
        if value_of_bet_on_team2 > self.cash_to_bet:
            raise Exception("Bet Value bigger than the cash to bet defined in class")

        if bet_team1_house1:
            return self._decimal_team2_house2*value_of_bet_on_team2 - self.cash_to_bet
        else:
            return self._decimal_team2_house1*value_of_bet_on_team2 - self.cash_to_bet

    def least_possible_value_team1(self, bet_team1_house1 = True):
        if bet_team1_house1:
            if self.can_bet_team1_house1:
                decimal_value = self._decimal_team1_house1
            else:
                raise Exception("This bet doesn't profit team 1 in house 1")
        else:
            if self.can_bet_team1_house2:
                decimal_value = self._decimal_team1_house2
            else:
                raise Exception("This bet doesn't profit team 1 in house 2")

        return (self.cash_to_bet/decimal_value)

    def biggest_possible_value_team1(self, bet_team1_house1 = True):
        if bet_team1_house1:
            if self.can_bet_team1_house1:
                decimal_value = self.decimal_team2_house2
            else:
                raise Exception("This bet don't profit team 1 in house 1")
        else:
            if self.can_bet_team1_house2:
                decimal_value = self.decimal_team2_house1
            else:
                raise Exception("This bet don't profit team 1 in house 2")

        return self.cash_to_bet - (self.cash_to_bet/decimal_value)
